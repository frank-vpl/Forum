<flux:dropdown position="bottom" align="start">
    <flux:sidebar.profile :name="auth()->user()->name" :initials="auth()->user()->initials()"
        :src="auth()->user()->profile_image_url ?? null" icon:trailing="chevrons-up-down"
            ...
            <div class="grid flex-1 text-start text-sm leading-tight">
                <flux:heading class="truncate flex items-center gap-1">
                    ...
                    @if (auth()->user()->getBadgeIconPath())
                        <img src="{{ auth()->user()->getBadgeIconPath() }}" alt="{{ auth()->user()->getBadgeTooltip() }}"
                            class="w-4 h-4" title="{{ auth()->user()->getBadgeTooltip() }}">
                    @endif
                </flux:heading>
