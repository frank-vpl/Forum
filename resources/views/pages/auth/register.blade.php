<x-layouts::auth>
    <div class="flex flex-col gap-6">
        ...
        <a href="{{ url('/redirect/x').(request('redirect') ? ('?redirect='.urlencode(request('redirect'))) : '') }}" class="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-300 dark:border-zinc-700 px-4 py-2 text-sm text-zinc-900 dark:text-white hover:bg-zinc-50 dark:hover:bg-zinc-800">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1227" class="h-4 w-4" fill="currentColor">
        ...
        <div class="space-x-1 rtl:space-x-reverse text-center text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <flux:link :href="route('login', ['redirect' => request('redirect')])" wire:navigate>{{ __('Log in') }}</flux:link>
        </div>
    </div>
</x-layouts::auth>
